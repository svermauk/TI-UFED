...
ALLOCATE(ds2(3))
ALLOCATE(ss(3),diff_s2(3))
ALLOCATE(hill(3,mtd_steps),width(3,mtd_steps))
...
DO i_mtd=1,mtd_steps
 READ(12,*) dum,hill(1,i_mtd),hill(2,i_mtd),hill(3,i_mtd),width(1,i_mtd),width(2,i_mtd),width(3,i_mtd),ht(i_mtd),dum
...
WRITE(*,*) 'calculating  c(t)'
 ...
 DO i_s3=1,nbin3
    grid(3,i_s3)=gridmin3+dfloat(i_s3-1)*griddiff3
 END DO

ALLOCATE(fes1(nbin1,nbin2,nbin3))

        fes1=0.d0
      DO i_mtd=1,mtd_steps
        ds2(1:3)=width(1:3,i_mtd)*width(1:3,i_mtd)
        ss(1:3)=hill(1:3,i_mtd)
        hh = ht(i_mtd)
...
!                if (diff_s2(2) .lt.-3.14d0 ) diff_s2(2) =diff_s2(2) + 6.28d0
                 ...
                 diff_s2(1:3)=diff_s2(1:3)*diff_s2(1:3)*0.5D0
 
!                fes1    = alpha*Vb

                 fes1(i_s1,i_s2,i_s3)=fes1(i_s1,i_s2,i_s3)-hh*DEXP((-diff_s2(1)/ds2(1))+(-diff_s2(2)/ds2(2))+(-diff_s2(3)/ds2(3)))

                 num=num+DEXP(-fes1(i_s1,i_s2,i_s3)/kt)
!                den=den+DEXP(-fes1(i_s1,i_s2,i_s3)/ktb)
                 den=den+DEXP(-fes1(i_s1,i_s2,i_s3)/kt + fes1(i_s1,i_s2,i_s3)*gamma_/kt)
              END DO 
           END DO
        END DO
