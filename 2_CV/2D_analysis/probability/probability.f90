...
ALLOCATE(cv1(md_steps),cv2(md_steps),dhdl(md_steps))
ALLOCATE(vbias(md_steps),ct(mtd_steps))

DO i_md=1,t_max
   READ(11,*) dum, cv1(i_md),cv2(i_md),vbias(i_md)
...
ALLOCATE(av_dhdl(nbin1,nbin2))
ALLOCATE(norm_dhdl(nbin1,nbin2))
...
DO i_md=1,md_steps
 IF((i_md.GT.t_min).AND.(i_md.LE.t_max))THEN
    ...
    IF(index1.gt.0.and.index2.gt.0.and.index1.le.nbin1.and.index2.le.nbin2) then
       if((gridmin1.le.cv1(i_md)).and.(gridmin2.le.cv2(i_md)).and.(gridmax1.ge.cv1(i_md)).and.(gridmax2.ge.cv2(i_md))) then
...
!          dum=dum*kj_to_kcal                                            ! Use this when vbias and ct is calculated from PLUMED
          prob(index1,index2) = prob(index1,index2) +  DEXP(dum/kt0)
          av_dhdl(index1,index2)=av_dhdl(index1,index2)+dhdl(i_md)
          norm_dhdl(index1,index2)=norm_dhdl(index1,index2)+1
...
!           prob(i_s1,i_s2)=prob(i_s1,i_s2)/den
            prob1=prob(i_s1,i_s2)
            dum1=dfloat(norm_dhdl(i_s1,i_s2)) !dum1 has the number of visits on a given bin
            data1=0.d0
            if (prob1 > 0.0) data1 = av_dhdl(i_s1,i_s2)/dum1 ! Local average <du/dl> in bin
            if(prob1+1.eq.prob1) prob1=1.0d-16  !remove infinity
            phi_z = -(kt)*dlog(prob1/(den*griddiff1*griddiff2))
