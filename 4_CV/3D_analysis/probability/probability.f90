...
ALLOCATE(cv1(md_steps),cv2(md_steps),cv3(md_steps),cv4(md_steps),dhdl(md_steps))
ALLOCATE(vbias(md_steps),ct(mtd_steps))

DO i_md=1,t_max
   READ(11,*) dum, cv1(i_md),cv2(i_md),cv3(i_md),cv4(i_md),vbias(i_md)
...
ALLOCATE(norm_dhdl(nbin1,nbin2,nbin3,nbin4))
...
DO i_md=1,md_steps
 IF((i_md.GT.t_min).AND.(i_md.LE.t_max))THEN
    ...
    if(index4.eq.nbin4)index4=1 ! WARNING! this is to avoid -pi and +pi bins to be counted seperately
    IF(index1.gt.0.and.index2.gt.0.and.index3.gt.0.and.index4.gt.0.and.                                                      &
       index1.le.nbin1.and.index2.le.nbin2.and.index3.le.nbin3.and.index4.le.nbin4) then
       if((gridmin1.le.cv1(i_md)).and.(gridmin2.le.cv2(i_md)).and.(gridmin3.le.cv3(i_md)).and.(gridmin4.le.cv4(i_md)).and.   &
          (gridmax1.ge.cv1(i_md)).and.(gridmax2.ge.cv2(i_md)).and.(gridmax3.ge.cv3(i_md)).and.(gridmax4.ge.cv4(i_md))) then
...
!          dum=dum*kj_to_kcal                                            ! Use this when vbias and ct is calculated from PLUMED
          prob(index1,index2,index3,index4) = prob(index1,index2,index3,index4) +  DEXP(dum/kt0)
          av_dhdl(index1,index2,index3,index4)=av_dhdl(index1,index2,index3,index4)+dhdl(i_md)
          norm_dhdl(index1,index2,index3,index4)=norm_dhdl(index1,index2,index3,index4)+1
...
!           prob(i_s1,i_s2)=prob(i_s1,i_s2)/den
            prob1=prob(i_s1,i_s2,i_s3,i_s4)
            dum1=dfloat(norm_dhdl(i_s1,i_s2,i_s3,i_s4)) !dum1 has the number of visits on a given bin
            data1=0.d0
            if (prob1 > 0.0) data1 = av_dhdl(i_s1,i_s2,i_s3,i_s4)/dum1 ! Local average <du/dl> in bin
            if(prob1+1.eq.prob1) prob1=1.0d-16  !remove infinity
            phi_z = -(kt)*dlog(prob1/(den*griddiff1*griddiff2*griddiff3*griddiff4))
...
DEALLOCATE(cv1, cv2, cv3, cv4, vbias, ct)
DEALLOCATE(prob)
DEALLOCATE(av_dhdl)
